...
const firebase = admin.initializeApp(functions.config().firebase, "statsGetV2");
...
const getMoodsWithTags = (
  moods: Mood[],
  tags: string[],
  tagType: string
): Mood[] => {
  return moods.filter((mood: Mood) => containsAnyTag(mood, tags, tagType));
};
...
const containsAnyTag = (
  mood: Mood,
  tags: string[],
  tagType: string
): boolean => {
  return tags.some((tag: string) => containsTag(mood, tag, tagType));
};
